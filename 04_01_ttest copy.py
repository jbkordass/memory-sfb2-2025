...
#def permute(perm_idx, a, b):
    ...
    try:
        ...
        print(f"[Fehler bei Permutation {perm_idx}]: {e}")
    return np.nan 
...
for ROI in ROIs:
    ...
    if do_permute:
        ...
        results = Parallel(n_jobs=n_jobs, verbose=10)(delayed(permute)( # Parallel computation to speed up the process
                             i, tfr_stim_ca_dat, tfr_sham_an_dat,perm_n)       # Permute data labels and compute t-test
                             for i in range(perm_n))
    ...
    diff_tfr = tfr_stim_an.copy()
    diff_tfr.data = tfr_stim_an.data - tfr_sham_an.data
    diff_tfr.plot(vmin=vmin/2, vmax=vmax/2, axes=axes[0,2],mask_style="contour", mask=mask_2d)
    ...
    #                                mask_style="contour", mask=mask)
    diff_tfr = tfr_stim_ca.copy()
    diff_tfr.data = tfr_stim_ca.data - tfr_sham_an.data
    diff_tfr.plot(vmin=vmin/2, vmax=vmax/2, axes=axes[0,2],mask_style="contour", mask=mask_2d)
